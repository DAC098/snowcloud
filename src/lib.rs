...
//! ## Traits
//!
//! to help with using a generator in other situations, traits are provided and implemented for the base types (more can be added later if nncessary/desired).
//!
//! - [`IdGenerator`](crate::traits::IdGenerator) describes the basic layout of an id generator. requiring an Error, Id, and Output type to be specified along with the next_id method.
//! - [`IdGeneratorMut`](crate::traits::IdGeneratorMut) is similar to [`IdGenerator`](crate::traits::IdGenerator) except the next_id call allows for mutating the object
//! - [`NextAvailId`](crate::traits::NextAvailId) describes an object that is capable of returing a [`duraiton`](std::time::Duration) to the next available millisecond.
//!
//! current use case would be for allowing different types of waiting for the next available id. see [`blocking_next_id`](crate::wait::blocking_next_id) for example implementation.
//!
//! ## Snowflake Bits
//!
...
//! ### Timestamp
//!
//! the timestamp is in millisconds and is based from a specific start date that you can specify. the start date must be in the future of [`UNIX_EPOCH`](std::time::SystemTime::UNIX_EPOCH) and cannot be a date in the future, `now >= start_time >= UNIX_EPOCH`. internally, a snowcloud will use [`SystemTime`](std::time::SystemTime) to get the timestamp and the convert to the necessary values.
//!
//! ```rust
